// --- /pages/settings.js ---
import { useState as useStateSettings, useEffect } from 'react';
import { useSession as useSettingsSession } from 'next-auth/react';
import HeaderSettings from '../components/Header';
import LoadingSpinnerSettings from '../components/LoadingSpinner';

export default function SettingsPage() {
    const { data: session, status } = useSettingsSession({ required: true });
    const [settings, setSettings] = useStateSettings({ x_api_key: '', facebook_token: '' });
    ...
    if (status === 'loading' || isLoading) {
        return <div className="min-h-screen flex items-center justify-center"><LoadingSpinnerSettings /></div>;
    }
    return (
